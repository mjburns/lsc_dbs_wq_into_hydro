...
#
# NEW REQUEST (this version):
#  - For DBS0004 panel ONLY: when plotting pipes, ONLY plot pipe segments that
#    fall WITHIN the DBS0004 catchment polygon (spatial clip).
#  - Leave L4 pipes behaviour as-is (it already looks good).
# ============================================================
...
# ---------------------------------------------------
# 1) Upstream pipes helper (no allupstream dependency)
# ---------------------------------------------------
...
# ---------------------------------------------------
# 4) Load streams from Postgres (same philosophy as Fig1)
# ---------------------------------------------------
db_m <- RPostgres::dbConnect(
  RPostgres::Postgres(),
  dbname   = "mwstr",
...
# ---------------------------------------------------
# 5) Pipe subsetting that actually works (robust)
# ---------------------------------------------------
subset_pipes_for <- function(pipes_sf, ids, subcs_df){
  ...
  # Try by pipeID field in pipes
  if ("pipeID" %in% names(pipes_sf)) {
    out <- pipes_sf[pipes_sf$pipeID %in% ids, ]
  }

  # If empty, try by pipeName field matching subcs pipeName for those ids
  ...
  # If STILL empty, fall back to all pipes (still clipped by extents)
...
# ---------------------------------------------------
# 5.5) NEW: clip pipes to DBS0004 polygon ONLY (spatial)
# ---------------------------------------------------
clip_lines_to_polygon <- function(lines_sf, poly_sf){
  ...
  # Union polygon (single mask)
  mask <- sf::st_union(sf::st_geometry(poly_sf))

  # Use intersection (keeps segments inside)
  # st_intersection can create GEOMETRYCOLLECTION; keep LINESTRING/MULTILINESTRING
  clipped <- suppressWarnings(sf::st_intersection(lines_sf, mask))

  if (!inherits(clipped, "sf") || nrow(clipped) == 0) {
    # If intersection yields nothing, return empty (because you asked "make sure pipes within polygon")
    return(lines_sf[0, ])
  }
...
# Apply only to DBS0004 panel pipes
pipes_D4e <- clip_lines_to_polygon(pipes_D4e, cat_D4e)

# ---------------------------------------------------
# 6) Panel extents: catchment-driven + padding
# ---------------------------------------------------
...
# ---------------------------------------------------
# 7) SCM plotting helper (points with small jitter)
# ---------------------------------------------------
...
# ---------------------------------------------------
# 8) Export device (bigger canvas + 600 dpi)
# ---------------------------------------------------
dir.create("images", showWarnings = FALSE, recursive = TRUE)
out_file <- "images/SCM_D4e_vs_L4_streams_pipes_catchments_LEGEND_RIGHT_v5_clipDBS0004pipes.tiff"
...
# ---------------------------------------------------
# 9) Panel plotter
#    Axis titles EXACTLY your requested style
# ---------------------------------------------------
plot_panel <- function(title_txt, ext, streams_sf, pipes_sf, cats_sf, scms_sf){

  # LEFT MARGIN provides dedicated space for Northing title
  par(mar = c(5.0, 8.6, 2.4, 1.0),
      mgp = c(2.7, 0.55, 0),
      xaxs = "i", yaxs = "i", las = 1)

  # Base plot (no auto axes/labels)
  ...
  # Draw layers (streams then pipes then catchments then SCMs)
  ...
  plot_scms_jittered(scms_sf,
                     cex = scm_cex,
                     jitter_m = scm_jitter_m,
                     seed = scm_jitter_seed)
...
plot_panel("D4-e (pipe 101)",     ext_D4e, streams, pipes_D4e, cat_D4e, SCMs_D4e)
plot_panel("L4 system (pipe 71)", ext_L4,  streams, pipes_L4,  cat_L4,  SCMs_L4)

# ---------------------------------------------------
# 10) Compact right-hand legend (no massive whitespace)
# ---------------------------------------------------
...
# Manual compact legend: tight y spacing
