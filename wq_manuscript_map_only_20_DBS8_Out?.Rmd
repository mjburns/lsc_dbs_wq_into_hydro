...
#
# CHANGE (this version):
#  - Keep everything else IDENTICAL to your v6 baseline (as pasted)
#  - Add arrows from LABEL -> SITE (same logic style as big Fig1 map)
#  - Reduce font size of axis titles (Easting/Northing)
# ============================================================
...
# ---------------------------------------------------
# 0) STYLE (baseline, with smaller axis-title font)
# ---------------------------------------------------
# Streams
col_stream <- "dodgerblue"
lty_stream <- 1
...
# Catchments
catch_lty <- 3
catch_lwd <- 2.4  # thicker boundary line

# Axes / labels
cex_axis       <- 1.05  # tick labels
cex_axis_title <- 1.18  # <-- reduced from 1.55
...
scm_cex  <- 0.62      # smaller points
scm_jitter_m <- 10    # metres (set 0 to disable)
scm_jitter_seed <- 71 # reproducible
...
# ---------------------------------------------------
# 1) HELPERS (same as big map for labels)
# ---------------------------------------------------
halo_text <- function(x, y, labels, cex = 1, font = 2,
                      col = "black", halo_col = "white", halo_mult = 0.75, ...) {
  offs <- halo_mult * strwidth("M", cex = cex) / 10
...
# Arrow from label (from) -> site (to), with gentle offset so the arrow doesn't start inside text
arrow_label_to_site <- function(x_from, y_from, x_to, y_to, lwd = 1, length = 0.05, col = "black"){
  usr <- par("usr")
  dx <- usr[2] - usr[1]
  dy <- usr[4] - usr[3]
  # small step away from label point
...
# ---------------------------------------------------
# 2) Upstream pipes helper (no allupstream dependency)
# ---------------------------------------------------
...
# ---------------------------------------------------
# 4) CRS sanity: make EVERYTHING share a CRS
# ---------------------------------------------------
...
# ---------------------------------------------------
# 6) Pipe subsetting that actually works (robust)
# ---------------------------------------------------
...
# ---------------------------------------------------
# 8) SCM plotting helper (points with small jitter)
# ---------------------------------------------------
...
out_file <- "images/SCM_D4e_vs_L4_streams_pipes_catchments_sites_labels_arrows_LEGEND_RIGHT_v7.tiff"
...
# Layout: two maps + slim right legend column
layout(matrix(c(1,2,3), nrow = 1, byrow = TRUE),
       widths = c(1.0, 1.0, 0.20))

# ---------------------------------------------------
# 10) Panel plotter (baseline + arrows label->site)
# ---------------------------------------------------
plot_panel <- function(title_txt, ext, streams_sf, pipes_sf, cats_sf, scms_sf,
  ...
  # ---- SITES + LABELS (added late; Fig1 style) ----
  ...
  if (inherits(labs_i, "sf") && nrow(labs_i) > 0) {
    ...
    # Draw label text
    halo_text(xy_lab[,1], xy_lab[,2], lab_txt,
              cex = cex_site_lab, font = 2)

    # Arrows: match by sitecode, label -> site centroid
    if (inherits(sites_i, "sf") && nrow(sites_i) > 0 && "sitecode" %in% names(labs_i) && "sitecode" %in% names(sites_i)) {
      # use centroid of the point geometry (safe even if it's a POINT already)
      site_xy <- sf::st_coordinates(sites_i)

      # for each label row, find corresponding site row
  ...
  # Axis titles EXACTLY as requested (but smaller cex now)
  mtext("Easting (m)",  side = 1, line = 3.4, font = 2, cex = cex_axis_title)
  mtext("Northing (m)", side = 2, line = 6.2, font = 2, cex = cex_axis_title, las = 0)
...
plot_panel("D4-e (pipe 101)",     ext_D4e, streams, pipes_D4e, cat_D4e, SCMs_D4e, sites_D4e)
plot_panel("L4 system (pipe 71)", ext_L4,  streams, pipes_L4,  cat_L4,  SCMs_L4,  sites_L4)

# ---------------------------------------------------
# 11) Compact right-hand legend (unchanged)
# ---------------------------------------------------
par(mar = c(0.8, 0.6, 0.8, 0.6))
plot.new()
box(col = "grey60", lwd = 0.9)

usr <- par("usr")

text(usr[1] + 0.08*(usr[2]-usr[1]), usr[4] - 0.06*(usr[4]-usr[3]),
     "Legend", adj = c(0,1), font = 2, cex = 0.95)

x0 <- usr[1] + 0.10*(usr[2]-usr[1])
y0 <- usr[4] - 0.18*(usr[4]-usr[3])
dy <- 0.11*(usr[4]-usr[3])

text(x0, y0, "SCMs", adj = c(0,1), font = 2, cex = 0.85)

y <- y0 - 0.90*dy
points(x0, y, pch = 19, col = scm_cols["tank"], cex = 0.85)
text(x0 + 0.12*(usr[2]-usr[1]), y, "Tank", adj = c(0,0.5), cex = 0.82)

y <- y - dy
points(x0, y, pch = 19, col = scm_cols["rg"], cex = 0.85)
text(x0 + 0.12*(usr[2]-usr[1]), y, "Raingarden", adj = c(0,0.5), cex = 0.82)

y <- y - dy
points(x0, y, pch = 19, col = scm_cols["dd"], cex = 0.85)
text(x0 + 0.12*(usr[2]-usr[1]), y, "Downpipe diverter", adj = c(0,0.5), cex = 0.82)

y <- y - 1.25*dy
text(x0, y, "Network", adj = c(0,1), font = 2, cex = 0.85)

y <- y - 0.95*dy
segments(x0, y, x0 + 0.16*(usr[2]-usr[1]), y, col = col_stream, lwd = lwd_stream, lty = lty_stream)
text(x0 + 0.22*(usr[2]-usr[1]), y, "Streams", adj = c(0,0.5), cex = 0.82)

y <- y - dy
segments(x0, y, x0 + 0.16*(usr[2]-usr[1]), y, col = col_pipe, lwd = lwd_pipe, lty = lty_pipe)
text(x0 + 0.22*(usr[2]-usr[1]), y, "Stormwater pipes", adj = c(0,0.5), cex = 0.82)
