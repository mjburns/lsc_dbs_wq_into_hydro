...
# ============================================================
# FIG 1 — FULL SCRIPT (centred extents, smaller axis titles,
#                      bigger tick labels)
# Changes requested:
#  - decrease font size of Easting/Northing titles
#  - increase axis tick label size
#  - better centre main map content (auto xlim/ylim; no forced xmin)
# ============================================================
...
# (keep simple here; if you still get topology errors, we can wrap in make_valid helpers)
...
# 2) OUTPUT DEVICE
# ---------------------------------------------------
out_file <- "images/Fig1_final_revised_centered.tiff"
...
pt_lwd_site     <- 1.2
cex_pts         <- 1.70   # a touch bigger

# Text
CEX_BOLD       <- 1.05
cex_axis       <- 1.20    # ↑ tick labels (requested)
cex_site_lab   <- 1.20
cex_key        <- 1.05
cex_axis_title <- 0.78    # ↓ axis titles (requested)
...
# ---------------------------------------------------
# 6) MAIN MAP EXTENTS (AUTO + centred)
# ---------------------------------------------------
# Compute bbox across layers we actually draw (helps centring and avoids odd whitespace)
catIA_2009 <- catIA[
...
# Catchment boundaries (uses your pre-made per-catchment border colours)
plot(catMap11_f$geom,
     lty = catch_lty, lwd = catch_lwd,
     border = sf::st_drop_geometry(catMap11_f)$col,
     add = TRUE)

# Sites (black outline matches map + legend via pt.lwd)
