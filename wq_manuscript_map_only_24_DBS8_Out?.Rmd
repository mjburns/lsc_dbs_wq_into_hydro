...
#
# CHANGE (this version):
#  - ONLY adjust the split in-panel legends on the 2nd (L4) panel:
#     * BOTH boxes flush to bottom corners (minimal padding)
#     * BOTH boxes wider so text never hits line samples
#     * SCM box can be shorter vertically
#     * Network box widened + better text offset from line samples
#  - Everything else identical to your last working script
# ============================================================
...
draw_scm_legend_br <- function(){
  ...
  # FLUSH bottom-right; WIDER, SHORTER
  box_w <- 0.46 * w     # wider to fit "Downpipe diverter"
  box_h <- 0.19 * h     # shorter vertically
  pad_x <- 0.010 * w    # nearly flush
  pad_y <- 0.012 * h
  ...
  # More interior horizontal room for labels
  xL   <- x1 + 0.06 * box_w
  xSym <- xL
  xTxt <- xL + 0.16 * box_w
...
draw_network_legend_bl <- function(){
  ...
  # FLUSH bottom-left; WIDER so "Stormwater pipes" fits comfortably
  box_w <- 0.44 * w
  box_h <- 0.16 * h
  pad_x <- 0.010 * w
  pad_y <- 0.012 * h
  ...
  # Longer line sample, and text pushed further right so it never overlaps
  line_len <- 0.26 * box_w
  gap_txt  <- 0.08 * box_w
