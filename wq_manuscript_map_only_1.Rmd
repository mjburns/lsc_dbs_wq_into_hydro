...
# ============================================================
# FIG 1 — FULL SCRIPT (FIXED legend pt.cex duplication)
# ============================================================
...
# Study catchments centroid (lon/lat)
...
# 2) OUTPUT DEVICE
# ---------------------------------------------------
out_file <- "images/Fig1_final_revised.tiff"
...
# Catchments (single source of truth)
...
cex_site_lab   <- 1.20
cex_key        <- 1.05
cex_axis_title <- 0.92  # smaller axis titles (requested)
...
draw_key_panel <- function(){
  ...
  # Catchments legend (FIX: only ONE pt.cex argument)
  legend("topleft", bty = "n", cex = cex_key,
         inset = c(0.06, 0.46),
         title = "Catchments",
         text.font = 1, title.font = 2,
  ...
  # Streams legend (exact match to map)
  legend("topleft", bty = "n", cex = cex_key,
         inset = c(0.06, 0.76),
         legend = "Streams",
...
draw_inset_australia <- function(){
  ...
  # Shade Victoria polygon (no red bbox)
...
# ---------------------------------------------------
# 6) MAIN MAP EXTENTS (no extra left padding)
# ---------------------------------------------------
xmin_fixed <- mean(c(345000, 350000))  # 347500

bb <- sf::st_bbox(sf::st_union(sf::st_geometry(catMap11_f)))
stopifnot(all(is.finite(as.numeric(bb))))

w_eff <- as.numeric(bb["xmax"] - xmin_fixed)
h_eff <- as.numeric(bb["ymax"] - bb["ymin"])
if (w_eff <= 0 || h_eff <= 0) stop("Invalid extent; check CRS and xmin_fixed.")

pad_right <- 0.05 * w_eff
pad_y     <- 0.07 * h_eff

xlim_pad <- c(xmin_fixed, as.numeric(bb["xmax"]) + pad_right)
ylim_pad <- c(as.numeric(bb["ymin"]) - pad_y, as.numeric(bb["ymax"]) + pad_y)
...
# Impervious (pre-Oct 2009)
catIA_2009 <- catIA[
  is.na(catIA$constructionDate) |
    (!is.na(catIA$constructionDate) & catIA$constructionDate < "2009-10-01"),
]
ia_cols <- c(col_conn, col_unconn, col_unconn)[match(catIA_2009$conn, c(1, 0, 0.5))]
plot(catIA_2009$geom, col = ia_cols, border = NA, lwd = lwd_minor, add = TRUE)

# Streams (exact style used in legend)
plot(streams$geom, col = col_stream, lwd = lwd_stream, lty = lty_stream, add = TRUE)

# Catchment boundaries — NOTE:
# If catMap11_f$col isn't exactly your desired palette, this will still match itself.
# If you want to FORCE borders to catch_cols by class, we need a class column.
plot(catMap11_f$geom,
     lty = catch_lty, lwd = catch_lwd,
     border = sf::st_drop_geometry(catMap11_f)$col,
     add = TRUE)

# Sites (exact match to legend)
plot(siteMap11_f$geom, pch = pch_site,
     bg = siteMap11_f$col, col = col_site_border,
...
mtext("Northing (m)", side = 2, line = 5.2, font = 2, cex = cex_axis_title, las = 0)
