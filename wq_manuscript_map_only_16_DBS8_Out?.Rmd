...
#
# CHANGE (this version):
#  - Keep everything else IDENTICAL to revert-v5
#  - Add SITE POINTS + SITE LABELS (same style as big Fig1 map)
#    * Left panel: DBS0004 only
#    * Right panel: LSS0001, LIS0001, LSN0001, LIS0004
# ============================================================
...
cex_axis_title <- 1.55  # axis titles (bigger than tick labels)
...
# ---------------------------------------------------
# 5) Load streams from Postgres (same philosophy as Fig1)
# ---------------------------------------------------
db_m <- RPostgres::dbConnect(
  RPostgres::Postgres(),
  dbname   = "mwstr",
...
out_file <- "images/SCM_D4e_vs_L4_streams_pipes_catchments_sites_labels_LEGEND_RIGHT_v6.tiff"
...
# ---------------------------------------------------
# 10) Panel plotter (UNCHANGED, plus sites+labels at end)
# ---------------------------------------------------
plot_panel <- function(title_txt, ext, streams_sf, pipes_sf, cats_sf, scms_sf,
  ...
  if (inherits(labs_i, "sf") && nrow(labs_i) > 0) {
    # match big-map behaviour: make font==1 bold
  ...
  # Axis titles EXACTLY as requested
  mtext("Easting (m)",  side = 1, line = 3.4, font = 2, cex = cex_axis_title)
  mtext("Northing (m)", side = 2, line = 6.2, font = 2, cex = cex_axis_title, las = 0)
