...
#
# CHANGE (this version):
#  - Reduce font size of axis titles (Easting/Northing) again
#  - Move legend INTO bottom-right of the 2nd (L4) panel (no separate legend box)
#  - Everything else identical to your v8 baseline (incl. label->site arrows)
# ============================================================
...
cex_axis_title <- 0.95  # <-- reduced again (was 1.05)
...
# Legend drawn inside current panel, bottom-right
draw_legend_in_panel <- function(){
  usr <- par("usr")
  w <- usr[2] - usr[1]
  h <- usr[4] - usr[3]

  # Box size (tuned to be compact)
  box_w <- 0.33 * w
  box_h <- 0.34 * h

  # Bottom-right placement with padding from edges
  pad_x <- 0.04 * w
  pad_y <- 0.05 * h
  ...
  rect(x1, y1, x2, y2, col = adjustcolor("white", alpha.f = 0.88),
  ...
  yT <- y2 - 0.08 * box_h
  text(xL, yT, "Legend", adj = c(0,1), font = 2, cex = leg_cex_title)

  # spacing within box
  y <- yT - 0.16 * box_h

  # Section: SCMs
  text(xL, y, "SCMs", adj = c(0,1), font = 2, cex = leg_cex_item)
  y <- y - 0.18 * box_h
  ...
  # Section: Network
  y <- y - 0.18 * box_h
  text(xL, y, "Network", adj = c(0,1), font = 2, cex = leg_cex_item)
  y <- y - 0.18 * box_h

  segments(xSym, y, xSym + 0.28 * box_w, y, col = col_stream, lwd = lwd_stream, lty = lty_stream)
  text(xTxt, y, "Streams", adj = c(0,0.5), cex = leg_cex_item)
  y <- y - 0.16 * box_h

  segments(xSym, y, xSym + 0.28 * box_w, y, col = col_pipe, lwd = lwd_pipe, lty = lty_pipe)
...
out_file <- "images/SCM_D4e_vs_L4_streams_pipes_catchments_sites_labels_arrows_LEGEND_INPANEL_v9.tiff"
...
# Layout: 2 panels only (legend now in-panel)
layout(matrix(c(1,2), nrow = 1, byrow = TRUE),
       widths = c(1.0, 1.0))

# ---------------------------------------------------
# 10) Panel plotter
#   - legend_in_panel: TRUE only for the 2nd plot
# ---------------------------------------------------
plot_panel <- function(title_txt, ext, streams_sf, pipes_sf, cats_sf, scms_sf,
  ...
  # In-panel legend (bottom-right), only when requested
  if (isTRUE(legend_in_panel)) {
    par(xpd = NA)
    draw_legend_in_panel()
    par(xpd = FALSE)
  }
