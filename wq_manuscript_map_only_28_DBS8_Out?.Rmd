...
# SCMs + streams (Postgres) + pipes + catchment boundaries
# + NEW: Impervious areas (ia) subset to upstream network for each panel
#
# CHANGE (this version):
#  - Add ia layer per panel:
#      * D4-e panel: ia$geom where ia$pipeID %in% upstream(pipe 101)
#      * L4 panel:   ia$geom where ia$pipeID %in% upstream(pipe 71)
#      * Colour scheme matches Fig1: Connected = gray(0.60), Unconnected = gray(0.85)
#        (conn values mapped via match(conn, c(1,0,0.5)) just like your big fig)
#  - Add an "Impervious areas" legend box ABOVE the SCM box (bottom-right stack)
#      * Items: Connected, Unconnected
#      * Same simple square-patch legend feel as the big fig
#  - Keep the existing SCM plot styling/legends intact otherwise
# ============================================================
...
# Impervious areas (MATCH big Fig1)
col_conn   <- gray(0.60)
col_unconn <- gray(0.85)
lwd_ia     <- 0.5
...
# --- Impervious plot helper (matches big figure mapping) ---
...
# Bottom-right SCM box (unchanged structure)
draw_scm_legend_br <- function(y_offset_frac = 0){
  usr <- par("usr")
  w <- usr[2] - usr[1]
  h <- usr[4] - usr[3]
  ...
  # allow vertical stacking by shifting up
  y_shift <- y_offset_frac * h

  x1 <- usr[2] - pad_x - box_w
  x2 <- usr[2] - pad_x
  y1 <- usr[3] + pad_y + y_shift
  y2 <- usr[3] + pad_y + box_h + y_shift
...
# NEW: Impervious areas legend box above SCM box (bottom-right stack)
draw_ia_legend_br <- function(){
  usr <- par("usr")
  w <- usr[2] - usr[1]
  h <- usr[4] - usr[3]

  # match SCM box width for clean stacking
  box_w <- 0.41 * w
  box_h <- 0.14 * h
  pad_x <- 0.010 * w
  pad_y <- 0.012 * h

  # place just above the SCM box with a tiny gap
  gap <- 0.010 * h

  # SCM box occupies: y in [usr3+pad_y, usr3+pad_y+0.19h]
  y1 <- usr[3] + pad_y + 0.19*h + gap
  y2 <- y1 + box_h
  ...
  # small filled squares like Fig1 (pch=15)
  points(xSym, y, pch = 15, col = col_conn,   cex = 1.05)
  text(xTxt, y, "Connected",   adj = c(0,0.5), cex = leg_cex_item)
  y <- y - 0.34 * box_h

  points(xSym, y, pch = 15, col = col_unconn, cex = 1.05)
  text(xTxt, y, "Unconnected", adj = c(0,0.5), cex = leg_cex_item)
}
...
draw_legends_split <- function(){
  ...
  draw_scm_legend_br()   # stays bottom-right
  draw_ia_legend_br()    # new box above it
}
...
out_file <- "images/SCM_D4e_vs_L4_streams_pipes_catchments_sites_labels_arrows_LEGEND_SPLIT_v13_withIA.tiff"
...
plot_panel <- function(title_txt, ext, streams_sf, pipes_sf, cats_sf, scms_sf,
  ...
  # NEW: Impervious areas layer (under streams/pipes/points)
