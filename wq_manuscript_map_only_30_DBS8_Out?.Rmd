...
# + Impervious areas (ia) subset to upstream network for each panel
#
# CHANGE (this version):
#  - Legends tightened HARD (less vertical whitespace in BOTH bottom-right boxes)
#  - No overlap: IA box sits above SCM box with explicit gap + smaller box heights
#  - ONE consistent legend font sizes for ALL 3 legend boxes (Network / IA / SCM)
# ============================================================
...
# ---------------------------------------------------
# 1.5) IN-PANEL LEGENDS (tight spacing; no overlap)
# ---------------------------------------------------
# NOTE: all three boxes use leg_cex_title / leg_cex_item so fonts match everywhere.
...
draw_scm_legend_br <- function(){
  ...
  box_w <- 0.41 * w
  box_h <- 0.158 * h     # tighter than before (was 0.19)
  pad_x <- 0.010 * w
  pad_y <- 0.012 * h
  ...
  # title closer to top; then small gap to first item
  y <- y2 - 0.10 * box_h
  ...
  points(xSym, y, pch = 19, col = scm_cols["tank"], cex = 0.72)
  text(xTxt, y, "Tank", adj = c(0,0.5), cex = leg_cex_item)
  y <- y - dy_item

  points(xSym, y, pch = 19, col = scm_cols["rg"], cex = 0.72)
  text(xTxt, y, "Raingarden", adj = c(0,0.5), cex = leg_cex_item)
  y <- y - dy_item

  points(xSym, y, pch = 19, col = scm_cols["dd"], cex = 0.72)
...
draw_ia_legend_br <- function(){
  ...
  box_w <- 0.41 * w
  box_h <- 0.118 * h     # tighter (was 0.14)
  pad_x <- 0.010 * w
  pad_y <- 0.012 * h

  gap <- 0.010 * h       # explicit non-overlap gap

  # compute from SCM box top so they never overlap
  scm_h <- 0.158 * h
  scm_y1 <- usr[3] + pad_y
  scm_y2 <- scm_y1 + scm_h
  ...
  points(xSym, y, pch = 15, col = col_conn, cex = 0.92)
  text(xTxt, y, "Connected", adj = c(0,0.5), cex = leg_cex_item)
  y <- y - dy_item

  points(xSym, y, pch = 15, col = col_unconn, cex = 0.92)
...
draw_network_legend_bl <- function(){
  ...
  box_w <- 0.44 * w
  box_h <- 0.145 * h     # slightly tighter than before (was 0.16)
  pad_x <- 0.010 * w
  pad_y <- 0.012 * h
...
plot_panel <- function(title_txt, ext, streams_sf, pipes_sf, cats_sf, scms_sf,
  ...
  # Impervious areas layer (under streams/pipes/points)
