...
#
# CHANGE (this version):
#  - Reduce font size of axis titles (Easting/Northing) a bit more
#  - Tidy legend: smaller right box, minimal whitespace, tighter layout
#  - Everything else identical to your v7 script (incl. label->site arrows)
# ============================================================
...
cex_axis_title <- 1.05  # <-- reduced again (was 1.18)
...
# tighter legend drawer (minimal whitespace; draws inside existing plot.new())
draw_compact_legend <- function(){
  ...
  # A tighter internal frame (so the outer panel can be small without dead space)
  rect(usr[1] + 0.06*w, usr[3] + 0.06*h,
       usr[2] - 0.06*w, usr[4] - 0.06*h,
       border = "grey70", lwd = 0.9, col = NA)
  ...
  # Title
  text(xL, yT, "Legend", adj = c(0,1), font = 2, cex = 0.92)

  # Compact spacing
  y <- yT - 0.14*h

  # Section: SCMs
  text(xL, y, "SCMs", adj = c(0,1), font = 2, cex = 0.82)
  y <- y - 0.10*h
  ...
  points(xSym, y, pch = 19, col = scm_cols["tank"], cex = 0.82)
  text(xTxt, y, "Tank", adj = c(0,0.5), cex = 0.80)
  y <- y - 0.10*h

  points(xSym, y, pch = 19, col = scm_cols["rg"], cex = 0.82)
  text(xTxt, y, "Raingarden", adj = c(0,0.5), cex = 0.80)
  y <- y - 0.10*h

  points(xSym, y, pch = 19, col = scm_cols["dd"], cex = 0.82)
  text(xTxt, y, "Downpipe diverter", adj = c(0,0.5), cex = 0.80)

  # Section: Network
  y <- y - 0.14*h
  text(xL, y, "Network", adj = c(0,1), font = 2, cex = 0.82)
  y <- y - 0.10*h

  # Streams line
  segments(xSym, y, xSym + 0.26*w, y, col = col_stream, lwd = lwd_stream, lty = lty_stream)
  text(xTxt, y, "Streams", adj = c(0,0.5), cex = 0.80)
  y <- y - 0.10*h

  # Pipes line
  segments(xSym, y, xSym + 0.26*w, y, col = col_pipe, lwd = lwd_pipe, lty = lty_pipe)
  text(xTxt, y, "Stormwater pipes", adj = c(0,0.5), cex = 0.80)
}
...
out_file <- "images/SCM_D4e_vs_L4_streams_pipes_catchments_sites_labels_arrows_LEGEND_TIGHT_v8.tiff"
...
# Layout: two maps + SMALLER legend column
layout(matrix(c(1,2,3), nrow = 1, byrow = TRUE),
       widths = c(1.0, 1.0, 0.14))
...
# ---------------------------------------------------
# 11) Tight legend panel (smaller box + minimal whitespace)
# ---------------------------------------------------
