...
# ============================================================
# ONLY CHANGE (as requested):
#  - For EACH panel, draw BOTH x- and y-axes manually with EXACTLY 3 ticks
#    (evenly spaced across that panel's xlim/ylim).
#  - This replaces the previous special-case x-axis fix for the L4 panel.
...
# --- NEW helper: exactly 3 ticks spanning a range (used for BOTH axes, BOTH panels) ---
three_ticks <- function(lim, round_to = 1000){
  lo <- lim[1]; hi <- lim[2]
  if (!is.finite(lo) || !is.finite(hi) || lo == hi) return(c(lo, lo, hi))

  # "pretty" endpoints aligned to round_to (keeps nice integers like 361000)
  ...
  mid <- (lo2 + hi2) / 2
  if (is.finite(round_to) && round_to > 0) {
    mid <- round(mid / round_to) * round_to
  }

  xt <- unique(c(lo2, mid, hi2))
  # ensure exactly 3 values (in case mid == lo2 or hi2 after rounding)
  if (length(xt) < 3) {
    # fall back to true midpoint (unrounded) to preserve 3
    mid2 <- (lo2 + hi2) / 2
    xt <- unique(c(lo2, mid2, hi2))
  }
  # final clamp to plot limits
  xt <- xt[xt >= lim[1] & xt <= lim[2]]
  # if clamping removed an end, use raw ends
  if (length(xt) < 3) {
    mid3 <- (lim[1] + lim[2]) / 2
...
plot_panel <- function(title_txt, ext, streams_sf, pipes_sf, cats_sf, scms_sf,
  ...
  # ---- AXES (ONLY CHANGE: both axes are now exactly 3 ticks, per panel) ----
  xt <- three_ticks(ext$xlim, round_to = 1000)  # keeps nice numbers like 361000
  yt <- three_ticks(ext$ylim, round_to = 1000)

  axis(1, at = xt, labels = format(xt, scientific = FALSE, trim = TRUE),
       cex.axis = cex_axis, las = 1, padj = 0.25)
  axis(2, at = yt, labels = format(yt, scientific = FALSE, trim = TRUE),
